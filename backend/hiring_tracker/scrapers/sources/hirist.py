...
logger = logging.getLogger("hiring_tracker.scrapers.hirist")
...
class HiristScraper(BaseScraper):
    ...
    ENDPOINTS = [
        "https://www.hirist.tech/k/data-scientist-jobs",
        "https://www.hirist.tech/k/machine-learning-jobs",
        "https://www.hirist.tech/k/ai-jobs",
    ]
    ...
    def run(self) -> List[NormalizedListing]:
        ...
        for url in self.ENDPOINTS:
            try:
                ...
                logger.warning("Failed to scrape Hirist %s: %s", url, exc)
        return results
    ...
    def _scrape_page(self, url: str) -> List[NormalizedListing]:
        ...
        try:
            ...
            if not listings:
                cards = soup.select(".job-box") or soup.select(".job-list-item") or soup.select(".jobCard")
                for card in cards:
                    try:
                        role_tag = card.select_one(".job-title") or card.select_one("a.title") or card.select_one("h3")
                        company_tag = card.select_one(".company-name") or card.select_one(".company") or card.select_one(".recruiter")
                        link_tag = card.select_one("a[href*='/j/']") or card.select_one("a")
