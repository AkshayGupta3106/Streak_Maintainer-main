...
logger = logging.getLogger("hiring_tracker.scrapers.unstop")
...
class UnstopScraper(BaseScraper):
    ...
    ENDPOINTS = [
        ("https://unstop.com/jobs", "FULL_TIME"),
    ...
    def run(self) -> List[NormalizedListing]:
        ...
        for url, default_type in self.ENDPOINTS:
            try:
                ...
                logger.warning("Failed to scrape Unstop %s: %s", url, exc)
        return results
    ...
    def _scrape_page(self, url: str, default_type: str) -> List[NormalizedListing]:
        ...
        try:
            ...
            if not listings:
                cards = soup.select(".opportunity-card") or soup.select(".card") or soup.select(".opp-card")
                for card in cards:
                    try:
                        role_tag = card.select_one(".title") or card.select_one("h3") or card.select_one(".opp-title")
                        company_tag = card.select_one(".company") or card.select_one(".company-name") or card.select_one(".sub-title")
