...
export default function RegisterPage() {
  ...
  const [isGoogleMode, setIsGoogleMode] = useState(Boolean(location.state?.googleEmail));
  const [googleEmail, setGoogleEmail] = useState(location.state?.googleEmail || '');
  const [googleName, setGoogleName] = useState(location.state?.googleName || '');
  const [googleToken, setGoogleToken] = useState(location.state?.googleToken || '');
  ...
  const attemptRegister = useCallback(async (submission) => {
    ...
    try {
      ...
      if (isBackendUnavailableError(requestError)) {
        ...
        setNotice(
          `Backend is waking up. Retrying registration automatically in ${BACKEND_RETRY_INTERVAL_SECONDS} seconds.`,
        );
        setError('');
        isRetryingRef.current = false;
        return;
      ...
      clearRetryLoop();
      setError(requestError.response?.data?.detail || 'Failed to create account, try again.');
      setNotice('');
      setIsSubmitting(false);
    };
  }, [clearRetryLoop, navigate, register, googleAuth]);
  ...
  useEffect(() => {
    ...
    retryIntervalRef.current = window.setInterval(() => {
      ...
      if (remainingSeconds <= 0) {
        void attemptRegister(queuedFormRef.current);
        return;
      }
  ...
  }, [attemptRegister, clearRetryLoop, retryState.active]);
  ...
  const handleSubmit = async (event) => {
    ...
    if (isGoogleMode) {
      ...
      if (!form.username.trim() || !form.name.trim() || !form.password || !form.confirmPassword) {
      ...
      const client = window.google.accounts.oauth2.initTokenClient({
        ...
        callback: async (tokenResponse) => {
          if (tokenResponse && tokenResponse.access_token) {
            setIsSubmitting(true);
            try {
              // Call Google API to get profile data for immediate display in the prefill fields
              ...
              if (!email) {
                throw new Error('Google did not return email address.');
              }
      ...
      <div className="auth-form-pane">
        <div className="auth-card">
          ...
          <BackendWakeSlider
            ...
            message="Your registration request is queued locally and will keep retrying automatically for up to one minute."
          ...
          <form className="stack-form" onSubmit={handleSubmit}>
            {isGoogleMode ? (
              <>
                <div style={{ display: 'grid', gap: '0.2rem', marginBottom: '0.4rem' }}>
                  <span className="menu-label" style={{ fontSize: '0.75rem' }}>Google Account</span>
                  <input
                    value={googleEmail}
                    disabled
                ...
                <div style={{ display: 'grid', gap: '0.2rem', marginBottom: '0.4rem' }}>
                  <span className="menu-label" style={{ fontSize: '0.75rem' }}>Name</span>
                  <input
                    value={googleName}
                    disabled
                    placeholder="Name"
                ...
                <button 
                  ...
                  style={{ border: 'none', background: 'none', color: '#c4fa06', fontSize: '0.85rem', padding: '0.5rem', cursor: 'pointer' }}
                  onClick={handleSwitchToRegular}
                >
                  Or register with standard form
                </button>
              </>
            ) : (
              <>
